...
strictly an OBSERVER:
  ...
  * Every function draws on a COPY of the frame — never the array the detector
    holds (Layer 2 ROIs are numpy views; drawing on the original corrupts them).
...
Two views are exposed:
  ...
  * STATIC per-frame PNGs (`annotate_stage1`, `plot_stage1_projection`,
    ...
    `plot_beat_signal`). Cheap, used by layer3.py.
...
from layer3.stage1 import Stage1Result
...
def plot_raw_chart(notes, cal, save_path: str | Path) -> Path:
    """Piano-roll of the Layer 3 raw output: lanes vs time."""
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    ...
    ax.set_title(f"Layer 3 raw chart — {len(notes)} notes "
...
# =============================================================================
# Overlay video — per-frame composite of every Layer 3 decision
# =============================================================================
#
# Why this exists
# ---------------
...
# Design notes
# ------------
# * Pure observer — runs the full pipeline like Layer3Pipeline.run() but reads
...
def render_overlay_video(
    ...
    *,
    ...
    """Render a frame-range mp4 of Layer 3's per-frame decisions.

    Parameters
    ----------
    cal
        A resolved Calibration (see `resolve_calibration`).
    save_path
...
if __name__ == "__main__":
    ...
    if ":" in arg2:
        ...
        s1d = ProjectionDetector(pre.cal)
        s2d = TemplateMatcher(pre.cal)

        cap = cv2.VideoCapture(str(pre.cal.video_path))
        ...
        cv2.imwrite(f"stage1_overlay_f{want}.png",
                    annotate_stage1(raw_frame, pre.cal, s1r))
        plot_stage1_projection(s1r, pre.cal, f"stage1_projection_f{want}.png")
        cv2.imwrite(f"stage2_overlay_f{want}.png",
                    annotate_stage2(raw_frame, pre.cal, s1r, s2r))
