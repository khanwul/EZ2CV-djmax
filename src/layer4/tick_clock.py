"""
EZ2CV — Layer 4 / tick_clock : BPMSegment + TickClock (ms ↔ tick)
===============================================================================
...
class TickClock:
    """Single ms ↔ tick gateway for the whole Layer 4 pipeline.
...
if __name__ == "__main__":
    # constant BPM round-trip
    clk = TickClock.from_drafts(
        [BPMDraft(start_ms=0.0, end_ms=60_000.0,
                  bpm_start=120.0, bpm_end=120.0)],
        origin_ms=0.0)
    for ms in (0.0, 250.0, 500.0, 60_000.0):
        t = clk.ms_to_tick(ms)
        back = clk.tick_to_ms(t)
        print(f"  const  ms={ms:9.2f} → tick={t:9.2f} → ms={back:9.2f}")

    # linear ramp 120→180 over 60s
    clk = TickClock.from_drafts(
        [BPMDraft(start_ms=0.0, end_ms=60_000.0,
                  bpm_start=120.0, bpm_end=180.0)],
        origin_ms=0.0)
    for ms in (0.0, 15_000.0, 30_000.0, 60_000.0):
        t = clk.ms_to_tick(ms)
        back = clk.tick_to_ms(t)
        print(f"  ramp   ms={ms:9.2f} → tick={t:9.2f} → ms={back:9.2f}")
