...
class Layer4Result:
    ...
    @staticmethod
    def from_layer3(l3: Layer3Result) -> "Layer4Result":
        return _build_layer4_result(l3)
...
def _build_layer4_result(l3: Layer3Result) -> Layer4Result:
    cal = l3.cal
    if not l3.barlines:
    ...
    rec = reconstruct_barlines(l3.barlines, l3.beats)
    ...
    notes, snaps = _convert_and_snap_notes(l3, clock, ticks_per_global_measure)

    # --- 4. stats -----------------------------------------------------
    stats = _build_stats(notes, snaps, clock, l3, bl_ticks, variants)
...
def _convert_and_snap_notes(l3: Layer3Result, clock: TickClock,
    ...
    raw_heads = [clock.ms_to_tick(n.trigger_ms) for n in l3.notes]
    ...
    for raw_note, raw_t, snap in zip(l3.notes, raw_heads, snaps):
    ...
    snaps_filtered = [snaps[i] for i, rn in enumerate(l3.notes)
